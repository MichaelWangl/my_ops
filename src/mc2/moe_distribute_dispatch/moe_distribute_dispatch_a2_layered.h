...
constexpr uint32_t BITS32_PER_BLOCK = UB_ALIGN / 4;
constexpr uint32_t BW_ITEM_SIZE = 32; // = sizeof(BatchWriteItem)
...
constexpr uint32_t INFO_NUM_IN_TOKENSTRUCT = 4; // 在Token后加入4种信息:expIds, weights, tokenIdx, scales
...
constexpr uint32_t EXP_TOKEN_COUNT_FLAG_CNT = UB_32B_ALIGN / sizeof(int32_t);
...
constexpr uint32_t RDMA_TOTAL_SIZE = 100U * 1024U * 1024U;
...
private:
    ...
    __aicore__ inline void CopyWin2Ipc();
    ...
    GlobalTensor<uint32_t> expertToServerGT_;
    ...
    GM_ADDR workspaceGM, TPipe *pipe, GM_ADDR tilingGM)
{
    ...
    totalWinSize_ = winContext_->winSize; // hccl windows总大小
    halfWinSize_ = RDMA_TOTAL_SIZE / 2;
    rdmaWinSize_ = halfWinSize_ - STATUS_SIZE_LAYERED;
    shareMemOffset_ = RDMA_TOTAL_SIZE;

    uint64_t winSizeMin = moeExpertNum_ * axisBS_ * (axisH_ * sizeof(XType) + INFO_NUM_IN_TOKENSTRUCT * axisK_ * sizeof(uint32_t)) + 
        IPC_DATA_OFFSET + RDMA_TOTAL_SIZE; // 考虑负载极其不均衡时，HCCL BUFFSIZE需要开的大小
    ...
    // struct相关信息初始化计算
    tokenStructLen_ = axisH_ * sizeof(ExpandXOutType) + INFO_NUM_IN_TOKENSTRUCT * (axisK_ * sizeof(uint32_t));
    tokenLenInStruct_ = axisH_ * sizeof(ExpandXOutType);
    expLenInStruct_ = axisK_ * sizeof(uint32_t);
    weightLenInStruct_ = axisK_ * sizeof(uint32_t);
    cntLenInStruct_ = axisK_ * sizeof(uint32_t);
    ...
    rankSizeOnIpc_ = (totalWinSize_ - RDMA_TOTAL_SIZE - IPC_DATA_OFFSET) / (localMoeExpertNum_ * worldSize_);
    rankSizeOnIpc_ = (rankSizeOnIpc_ / IPC_BUFF_ALIGN) * IPC_BUFF_ALIGN;

    aivId_ = GetBlockIdx();
    expertIdsCnt_ = axisBS_ * axisK_;
    serverNum = worldSize_ / SERVER_RANK_SIZE;
    serverSizeOnWin_ = rdmaWinSize_ / serverNum;
    serverSizeOnWin_ = (serverSizeOnWin_ / RDMA_BUFFER_ALIGN) * RDMA_BUFFER_ALIGN;

    bufferChosenGlobal_.SetGlobalBuffer((__gm__ uint32_t*)(windowInGM_ + rdmaWinSize_ + worldSize_ * STATE_OFFSET));
    ...
    sendStatusTensor_.SetGlobalBuffer((__gm__ int32_t*)(windowOutGM_ + rdmaWinSize_));
    readStatusTensor_.SetGlobalBuffer((__gm__ int32_t*)(windowInGM_ + rdmaWinSize_));

    expertTokenNumsOutGM_ = expertTokenNumsOut; // 无GlobalTensor
    epRecvCountsGM_ = epRecvCountsOut; // 无GlobalTensor
    statusSpaceGm_ = windowInGM_ + rdmaWinSize_;
    ...
    expertToServerGT_.SetGlobalBuffer((__gm__ uint32_t*)(expertToServerCntGM_),
    ...
    tpipe_->InitBuffer(expertIdsBuf_, expertIdsCnt_ * sizeof(int32_t)); 
...
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::CreateInnerReduceInfo()
{
    ...
    if (curServerId == rankId_ / SERVER_RANK_SIZE) {
        tokenCntGlobalAddr = (__gm__ uint8_t*)(windowOutGM_) + curServerId * serverSizeOnWin_;
    } else {
        tokenCntGlobalAddr = (__gm__ uint8_t*)(windowInGM_) + curServerId * serverSizeOnWin_;
    }
    ...
    LocalTensor<int32_t> localUB = tBuf.GetWithOffset<int32_t>(RoundUp(realBS * axisK_, BITS32_PER_BLOCK),
        baseBuffOffset);
    ...
    for (uint32_t tokenIdx=0; tokenIdx < realBS; tokenIdx++) {
        for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
            int32_t expId = localUB(tokenIdx * axisK_ + expIdx);
    ...
    for (uint32_t expBlockId=0; expBlockId < moeExpertNumInServer_; expBlockId++) {
        ...
        for (uint32_t tokenIdx=0; tokenIdx < validCnt; tokenIdx++) {
            ...
            for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
                uint32_t expId = localUB(tokenId * axisK_ + expIdx);
                if (expId >= currServerExpBegin && expId < currServerExpEnd) {
...
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::CreateOuterReduceInfo()
{
    ...
    DataCopy(expertIdsTensor_, expertIdsGMTensor_, expertIdsCnt_);
    ...
    for (uint32_t tokenIdx = 0; tokenIdx < axisBS_; ++tokenIdx) {
        ...
        for (uint32_t serverIdx = 0; serverIdx < serverNum; ++serverIdx) {
            ...
            outerCntLt(tokenIdx) += 1; // 这个token发往了几个server
...
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::ReorderTokens()
{
    ...
    uint32_t startTokenIdx = sendTokenNum * aivId_;
    // 分核，每个Core处理sendTokenNum个Token的遍历
    if (aivId_ < remainderTokenNum) { // 前remainderRankNum个aiv需要多发1个卡的数据
        sendTokenNum += 1;
        startTokenIdx += aivId_;
    } else {
        startTokenIdx += remainderTokenNum;
    }
    ...
    LocalTensor<int32_t> tokenTempTensorI32_ =
        tBuf.GetWithOffset<int32_t>(((tokenUbSize * sendTokenNum) / sizeof(int32_t)), TBUF_TEMP_OFFSET);
    ...
    expertIdsGMTensorU8_.SetGlobalBuffer((__gm__ uint8_t*)expertIdsGM_);
    GlobalTensor<uint8_t> weightGt;
    weightGt.SetGlobalBuffer((__gm__ uint8_t*)weightsGM_);

    DataCopy(expertIdsTensor_, expertIdsGMTensor_, expertIdsCnt_); // 专家List读取
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    ...
    if constexpr (DynamicQuant || StaticQuant) {
        ...
        DataCopyPad(tokenTempTensorU8_, xGMTensorU8_[startTokenIdx * axisH_ * sizeof(XType)], tokenCopyParams,
            tokenPadParams);
        ...
        DataCopyPadExtParams<uint8_t> tokenPadParams;
        DataCopyPad(tokenTempTensorU8_, xGMTensorU8_[startTokenIdx * tokenLenInStruct_], tokenCopyParams, tokenPadParams);
    }

    DataCopyExtParams expCopyParams{static_cast<uint16_t>(sendTokenNum), static_cast<uint16_t>(expLenInStruct_),
        0, static_cast<uint16_t>(infoGapInStruct_), 0};
    DataCopyPadExtParams<uint8_t> expPadParams;
    DataCopyPad(tokenTempTensorU8_[expOffsetInStruct_], expertIdsGMTensorU8_[startTokenIdx * expLenInStruct_],
                expCopyParams, expPadParams);

    // Weights进行拷贝
    DataCopyExtParams weightCopyParams{static_cast<uint16_t>(sendTokenNum), static_cast<uint16_t>(weightLenInStruct_),
        0, static_cast<uint16_t>(infoGapInStruct_), 0};
    DataCopyPadExtParams<uint8_t> weightPadParams;
    DataCopyPad(tokenTempTensorU8_[weightOffsetInStruct_], weightGt[startTokenIdx * weightLenInStruct_],
                weightCopyParams, weightPadParams);

    // ===========================遍历=========================== //
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    uint32_t startExpIdx = startTokenIdx * axisK_;
    uint32_t endExpIdx = startExpIdx + sendTokenNum * axisK_;
    for (uint32_t expertIndex = 0; expertIndex < endExpIdx; ++expertIndex) {
        uint32_t tokenIndex = expertIndex / axisK_;
        int32_t expertId = expertIdsTensor_(expertIndex);   // 读取expId
        int32_t currentSum = expertCountTensor_(expertId);  // 读取已经往该Exp发送了几个token
        uint32_t dstServerId = (uint32_t)expertId / moeExpertNumInServer_;  // 该token去往哪个Server
        expertToServerCntTensor_(tokenIndex * serverNum + dstServerId) = (int8_t)1; // 覆盖写，确保去往同一个Server，该Token只统计一次
        expertCountTensor_(expertId) = currentSum + 1; // 更新发往该专家的总token数
        
        if (expertIndex >= startExpIdx && expertIndex < endExpIdx) { // 本Aiv要处理的token信息需要特殊计算
            uint32_t sendTokenIdx = expertIndex / axisK_ - startTokenIdx; // 当前处理的Token在UB中的第几个
            uint32_t curCntOffset = (sendTokenIdx * tokenStructLen_ + cntOffsetInStruct_) / sizeof(uint32_t); // 索引到Cnt的位置
            tokenTempTensorU32_(curCntOffset + expertIndex % axisK_) = currentSum; // 写去往该Exp的token数,写在第ExpIdx位置
            expertToServerIdxTensor_(dstServerId) = expertIndex; // 覆盖写，记录本token中最后一个想去往该Server的Expidx
            ...
            for (uint32_t reviewExpIdx = expertIndex + 1 - axisK_; reviewExpIdx < expertIndex + 1; reviewExpIdx++) {
                int32_t reviewExpertId = expertIdsTensor_(reviewExpIdx); // 往前回顾axisK_个Expid
        ...
        if (expertIndex == expertIdsCnt_ - 1) {
            ...
            SyncFunc<AscendC::HardEvent::S_MTE3>();
            DataCopy(expertToServerGT_, expertToServerCntTensor_, RoundUp(axisBS_ * serverNum, B32_PER_BLOCK));
        }
    }
}

template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::QuantProcess(
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::CopyTokenToWinOut(
    uint32_t localTokenIdx, uint32_t globalTokenIdx, uint32_t dstServerId)
{
    ...
    SyncFunc<AscendC::HardEvent::S_MTE3>();
    uint32_t destOffset = dstServerId * serverSizeOnWin_ + tokenStructLen_ * toServerCntSum + TOKEN_COUNT_SIZE;
...
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::GatherAndWriteCntInfo()
{
    ...
    DataCopy(expertToServerCntTensor_, expertToServerGT_, RoundUp(axisBS_ * serverNum, B32_PER_BLOCK));
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    ...
    for (uint32_t dstServerId = startServerId; dstServerId < startServerId + destServerNum; ++dstServerId) {
        ...
        writeCntLt.SetValue(0, dstServerCnt); // windows最前面存放本卡发送到目标server一共多少个token
        uint32_t destOffset = (dstServerId * serverSizeOnWin_) / sizeof(uint32_t);
        
        SyncFunc<AscendC::HardEvent::S_MTE3>();
        DataCopy(sendTokensU32Tensor_[destOffset], writeCntLt, EXP_TOKEN_COUNT_FLAG_CNT); // 对齐拷贝token个数信息
    }
}
...
{
    uint32_t batchWriteItemNum = serverNum / aivNum_;
    uint32_t remainderItemNum = serverNum % aivNum_;
    uint32_t startServerId = batchWriteItemNum * aivId_;
    uint32_t curServerId = rankId_ / SERVER_RANK_SIZE;
    ...
    if (aivId_ < remainderItemNum) {
        ...
        startServerId += remainderItemNum;
    }
    ...
    for (uint32_t dstServerId = startServerId; dstServerId < endServerId; ++dstServerId) {
        ...
        uint64_t srcDataRdmaAddr = (uint64_t)(hccl_.GetWindowsOutAddr(rankId_) + halfWinSize_ * bufferId_ +
            dstServerId * serverSizeOnWin_);
        // 去往该Server的传输的数据量
        uint32_t validTokenCount = expertToServerIdxTensor_(dstServerId);
        ...
        uint64_t dstFlagRdmaAddr = (uint64_t)(hccl_.GetWindowsInAddr(dstRankId) + halfWinSize_ * bufferId_ +
            rdmaWinSize_ + curServerId * STATE_OFFSET);
        ...
        SyncFunc<AscendC::HardEvent::S_MTE3>();
        uint32_t dstServerOffset = dstServerId;
...
template <TemplateMC2TypeA2layeredClass>
__aicore__ inline void MoeDistributeDispatchA2Layered<TemplateMC2TypeA2layeredFunc>::CopyWin2Ipc()
{
    uint32_t coresPerServer = (aivNum_ - serverNum) / serverNum;
    if (aivId_ >= coresPerServer * serverNum) {
        return;
    }
    
    // 获取tokenCnt,计算本卡收到对端server多少Token，用于后续分核计算
    uint32_t fromServerId = aivId_ / coresPerServer; // 计算本core需要处理的ServerId
    __gm__ uint8_t *tokenCntGlobalAddr;
    if (fromServerId == rankId_ / SERVER_RANK_SIZE) {
        tokenCntGlobalAddr = (__gm__ uint8_t*)(windowOutGM_) + fromServerId * serverSizeOnWin_;
    } else {
        tokenCntGlobalAddr = (__gm__ uint8_t*)(windowInGM_) + fromServerId * serverSizeOnWin_;
    }
    ...
    uint32_t winInTokenOffset = fromServerId * serverSizeOnWin_ + TOKEN_COUNT_SIZE;
    uint32_t localAivId = aivId_ % coresPerServer;
    uint32_t tokenCntPerAiv = tokenCnt / coresPerServer;
    uint32_t tokenCntRemain = tokenCnt % coresPerServer;
    uint32_t tokenCntPreRemain = (localAivId < tokenCntRemain) ? localAivId : tokenCntRemain;
    uint32_t tokenCntCurAiv = (localAivId < tokenCntRemain) ? (tokenCntPerAiv + 1) : tokenCntPerAiv;

    LocalTensor<uint8_t> localUB = tBuf.GetWithOffset<uint8_t>(DISPATCH_TOKEN_UB_SIZE / sizeof(uint8_t), TBUF_TEMP_OFFSET);
    ...
    for (uint32_t batchIdx = 0; batchIdx < batchCnt; ++batchIdx) {
        ...
        for (uint32_t tokenIdx = 0; tokenIdx < tokenCntInBatch; ++tokenIdx) {
            ...
            for (uint32_t expIdx = 0; expIdx < axisK_; ++expIdx) {
                ...
                uint32_t targetExpOffset = (targetExpertId % localMoeExpertNum_) * worldSize_ * rankSizeOnIpc_;
                // 第几个Exp段
                uint32_t targetServerOffset = fromServerId * SERVER_RANK_SIZE * rankSizeOnIpc_;
                // 第几个Server段
                uint32_t targetRankOffset = (rankId_ % SERVER_RANK_SIZE) * rankSizeOnIpc_;
                // 第几个Rank段
                ...
                uint32_t setTokenCntOffset = (targetExpertId % localMoeExpertNum_) * worldSize_ +
                    fromServerId * SERVER_RANK_SIZE + (rankId_ % SERVER_RANK_SIZE);
...
#pragma unroll 8
    ...
    for (uint32_t idx = 0; idx < srCntCurCore; ++idx) {
        ...
        for (uint32_t batchIdx = 0; batchIdx < batchCnt; ++batchIdx) {
            ...
            uint32_t srcIpcOffset = srIdx * rankSizeOnIpc_ + batchIdx * tokenCntInUB * tokenStructLen_;
            ...
            for (uint32_t tokenIdx = 0; tokenIdx < tokenCntInBatch; tokenIdx++) {
                for (uint32_t expIdx = 0; expIdx < axisK_; expIdx++) {
                    ...
                    if (curExpIdx + rankId_ * localMoeExpertNum_ == localUBint32(expOffset)) {
                        uint32_t weightOffset = expOffset + axisK_;
            ...
            if constexpr (DynamicQuant) {
                DataCopyExtParams quantTokenParams{static_cast<uint16_t>(tokenCntInBatch),
                    ...
                    static_cast<uint32_t>(infoGapInStruct_), 0, 0};
