...
namespace optiling {
static ge::graphStatus MoeDistributeCombineA2CheckAttrAndSetTiling(gert::TilingContext *context, MoeDistributeCombineA2Info& info)
{
  ...
  OPS_CHECK(expertSharedTypePtr == nullptr, OPS_LOG_E(K_INNER_DEBUG, "expertSharedType is null."), return GRAPH_FAILED);
  OPS_CHECK(sharedExpertRankNumPtr == nullptr, OPS_LOG_E(K_INNER_DEBUG, "sharedExpertRankNum is null."), return GRAPH_FAILED);
                                                                    ...
                                                                    const bool isLayered)
{
  ...
  OPS_CHECK(k <= 0 || k > MAX_K_VALUE_A2,
  ...
  OPS_CHECK(isLayered && k != LAYERED_SUPPORT_K, 
    OPS_LOG_E(K_INNER_DEBUG, "As layered, K must be %u.", LAYERED_SUPPORT_K), return ge::GRAPH_FAILED);
  info.bs = bs;
  info.k = k;
  info.h = h;

  OPS_LOG_D(K_INNER_DEBUG, "batchSize=%d", bs);
  OPS_LOG_D(K_INNER_DEBUG, "k=%d", k);
  OPS_LOG_D(K_INNER_DEBUG, "hidenSize=%d", h);
...
static ge::graphStatus MoeDistributeCombineA2TilingFuncImpl(gert::TilingContext* context)
{
  ...
  OPS_CHECK(MoeDistributeCombineA2CheckShapeAndSetTiling(context, info, isLayered) != ge::GRAPH_SUCCESS,
    OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "MoeDistributeCombineA2 CheckShapeAndSetTiling Failed"),
    return ge::GRAPH_FAILED);
  OPS_CHECK(MoeDistributeCombineA2CheckAttrAndSetTiling(context, info) != ge::GRAPH_SUCCESS,
  ...
  OPS_CHECK(MoeDistributeCombineA2GetPlatformInfoAndSetTiling(context, info) != ge::GRAPH_SUCCESS,
    OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "MoeDistributeCombineA2 GetPlatformInfoAndSetTiling Failed"),
    return ge::GRAPH_FAILED);
