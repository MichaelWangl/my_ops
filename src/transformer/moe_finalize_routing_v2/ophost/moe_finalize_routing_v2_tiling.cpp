...
ge::graphStatus MoeFinalizeRoutingTilingV2::CheckParamsShape(ShapeParamsV2& params)
{
    ...
    OPS_ERR_IF(biasShape != nullptr && biasShape->GetStorageShape().GetDim(DIM_INDEX_0) < expandedExpertIdxShape->GetStorageShape().GetDim(DIM_INDEX_1),
        OPS_REPORT_VECTOR_INNER_ERR(context_->GetNodeName(),"E should be larger than or equal to K."),
        return ge::GRAPH_FAILED);
    
    OPS_ERR_IF(biasShape != nullptr && (biasShape->GetStorageShape().GetDim(DIM_INDEX_0) == 0),
