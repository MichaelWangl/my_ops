...
template <typename T>
__aicore__ inline void MoeInitRoutingV2GradDroplessCompute<T>::Init(GM_ADDR gradExpandedX,
  ...
  this->gradExpandedXGm.SetGlobalBuffer((__gm__ T*)gradExpandedX, this->n * this->k * this->cols);  // input: {B*S*K, H}
...
__aicore__ inline void MoeInitRoutingV2GradDroplessCompute<T>::GradProcessTokenAccumulate(
  ...
  for (; tokenIdx < tokenIdxEnd; tokenIdx += this->baseStride) {
    ...
    int64_t tmpBuffOffset =
        (this->multiTmpBuff) ?
        (tokenTmpBuffSizeOffset + this->perBuffSize * binIdx) : tokenTmpBuffSizeOffset;
...
__aicore__ inline void MoeInitRoutingV2GradDroplessCompute<T>::GradProcess(int64_t elementIdx,
  ...
  GradProcessTokenAccumulate(tokenIdx, tokenIdxEnd, tokenBinBuffSizeOffset, tokenTmpBuffSizeOffset, colOffset, cpyCols);

  // S3：拷贝加数，累加到对应的被加数buffer中
  GradProcessTokenAccumulate(tokenIdxStart + 1, tokenIdxEnd, tokenBinBuffSizeOffset,
                             tokenTmpBuffSizeOffset, colOffset,
                             cpyCols);
  ...
  for (int64_t expIdx = 0; expIdx < this->expNum; expIdx++) {
    ...
    for (int64_t j = stride; j < this->binBufferNum; j += interval) {
      ...
      int64_t bBuffOffset = tokenBinBuffSizeOffset +
            this->perBuffSize * j;
      LocalTensor<float> addALocal = 
          this->binBuff.template GetWithOffset<float>(this->perCpyCols, aBuffOffset);
      LocalTensor<float> addBLocal = 
          this->binBuff.template GetWithOffset<float>(this->perCpyCols, bBuffOffset);
...
__aicore__ inline void MoeInitRoutingV2GradDroplessCompute<T>::Process() {
  ...
  for (int64_t tokenGroupIdx = 0; tokenGroupIdx < this->tokensLoop; tokenGroupIdx++) {
    ...
    for (int64_t elementLoop = 0; elementLoop < tokenGroupSize; elementLoop++) {
      int64_t elementIdx = elementStartIdx + 
          tokenGroupIdx * this->tokensFormer + 
          elementLoop;  // 绝对位置
      int64_t tokenBinBuffSizeOffset =
