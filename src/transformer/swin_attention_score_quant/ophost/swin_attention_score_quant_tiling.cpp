...
{
    OPS_CHECK(qOriShape.GetDimNum() != QKV_DIM_SIZE, OPS_LOG_E(context->GetNodeName(),
...
inline bool CheckKVTensor(gert::TilingContext *context, gert::Shape &qOriShape)
{
    for (uint32_t i = K_IDX; i <= V_IDX; i++) {
        ...
        if (kvShape != nullptr) {
            ...
            OPS_LOG_E(context->GetNodeName(), "input tensor %d is nullptr\n", i);
            return false;
        }
    }
    return true;
...
inline bool CheckMaskDimSize(gert::TilingContext *context, gert::Shape &qOriShape, gert::Shape & maskOriShape)
{
    ...
    for (uint32_t i = 1; i < QKV_DIM_SIZE - 1; i++) {
        OPS_CHECK(qOriShape.GetDim(i) != maskOriShape.GetDim(i),
            OPS_LOG_E(context->GetNodeName(), "input dim[%d] of mask is wrong\n", i), return false);
    }
...
void TilingLog(SwinAttentionScoreQuantTilingData &tiling) {
    ...
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.M]: %u", tiling.qkBmmTilingData.get_M());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.N]: %u", tiling.qkBmmTilingData.get_N());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.Ka]: %u", tiling.qkBmmTilingData.get_Ka());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.Kb]: %u", tiling.qkBmmTilingData.get_Kb());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.singleCoreM]: %u", tiling.qkBmmTilingData.get_singleCoreM());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.singleCoreN]: %u", tiling.qkBmmTilingData.get_singleCoreN());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.singleCoreK]: %u", tiling.qkBmmTilingData.get_singleCoreK());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.baseM]: %u", tiling.qkBmmTilingData.get_baseM());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.baseN]: %u", tiling.qkBmmTilingData.get_baseN());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.baseK]: %u", tiling.qkBmmTilingData.get_baseK());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[qkBmm.isBias]: %u", tiling.qkBmmTilingData.get_isBias());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.M]: %u", tiling.pvBmmTilingData.get_M());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.N]: %u", tiling.pvBmmTilingData.get_N());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.Ka]: %u", tiling.pvBmmTilingData.get_Ka());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.Kb]: %u", tiling.pvBmmTilingData.get_Kb());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.singleCoreM]: %u", tiling.pvBmmTilingData.get_singleCoreM());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.singleCoreN]: %u", tiling.pvBmmTilingData.get_singleCoreN());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.singleCoreK]: %u", tiling.pvBmmTilingData.get_singleCoreK());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.baseM]: %u", tiling.pvBmmTilingData.get_baseM());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.baseN]: %u", tiling.pvBmmTilingData.get_baseN());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.baseK]: %u", tiling.pvBmmTilingData.get_baseK());
    OPS_LOG_I("[SwinAttentionScoreQuant]", "[pvBmm.isBias]: %u", tiling.pvBmmTilingData.get_isBias());
