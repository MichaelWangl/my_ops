...
ge::graphStatus MoeInitRoutingQuantTilingBase::GetShapeAttrsInfo()
{
    ...
    OPS_CHECK_NULL_WITH_CONTEXT(context_, activateNumPtr);
...
{
    OPS_LOG_I(opName, "moeInitRoutingTilingData is coreNum:%ld, n:%ld, cols:%ld, k:%ld, scale is: %f, offset is: %f",
...
void MoeInitRoutingQuantTilingBase::Tinlig4VBSMultiCoreCompute(QuantVBSComputeTilingData *tilingData)
{
    ...
    do {
        ...
        int64_t lastCorePerLoopElements = CeilDiv(
            CeilDiv(tilingData->get_lastCoreElements(), tilingData->get_lastCoreLoops()), SORT32_ALIGN_ELEMENT) *
    ...
    OPS_CHECK(tilingData->get_lastCoreLastLoopElements() <= 0,
        OPS_REPORT_VECTOR_INNER_ERR(opName, "vbs tiling failed"),
        ;);
}
...
void MoeInitRoutingQuantTilingBase::Tiling4SrcToDstCompute()
{
    ...
    if (perLoopMaxRows >= tilingData->get_lastCoreRows()) {
        ...
        tilingData->set_lastCoreLastLoopRows(tilingData->get_lastCoreRows() -
            (CeilDiv(tilingData->get_lastCoreRows(), perLoopMaxRows) - 1) * perLoopMaxRows);
    }
}
...
void MoeInitRoutingQuantTilingBase::Tiling4GatherOutComputeSplitK()
{
    ...
    int64_t perLoopMaxRows = (static_cast<int64_t>(aicoreParams_.ubSize) / NUM_TWO - kFactor * ONE_BLOCK_BYTE) /
        ...
        (maxColsOneLoop + ONE_BLOCK_BYTE) * (inuptXDtypeSize_ + static_cast<int64_t>(sizeof(float) + sizeof(int16_t))) +
        maxColsOneLoop * inuptXDtypeSize_ + ONE_BLOCK_BYTE);
    OPS_LOG_I(opName, "perLoopMaxRows is %ld", perLoopMaxRows);
    while (perLoopMaxRows <= 0) {
        OPS_LOG_I(opName, "perLoopMaxRows is %ld, less than 0", perLoopMaxRows);
...
ASCENDC_EXTERN_C ge::graphStatus TilingForMoeInitRoutingQuant(gert::TilingContext *context)
{
    MoeInitRoutingQuantTilingBase tiling(context);
    return tiling.DoTiling();
}

ASCENDC_EXTERN_C ge::graphStatus TilingPrepareForMoeInitRoutingQuant(gert::TilingParseContext *context)
{
    return ge::GRAPH_SUCCESS;
}
