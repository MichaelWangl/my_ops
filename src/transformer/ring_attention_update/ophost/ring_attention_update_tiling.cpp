...
static ge::graphStatus RingAttentionUpdateTNDUbSizeCheck(const gert::TilingContext* context,
  ...
  OPS_LOG_D(context->GetNodeName(), "headNum in TND = %ld", headNum);
  OPS_LOG_D(context->GetNodeName(), "headDim in TND = %ld", headDim);
  ...
  OPS_LOG_D(context->GetNodeName(), "attnEleNumLoop = %ld", attnEleNumLoop);
  OPS_LOG_D(context->GetNodeName(), "softmaxEleNumLoop = %ld", softmaxEleNumLoop);
  ...
  OPS_LOG_D(context->GetNodeName(), "Total UB Size Used = %ld", totalBufferSizeUsed);
  OPS_LOG_D(context->GetNodeName(), "Max UB size = %ld", MAX_UB_SIZE);
